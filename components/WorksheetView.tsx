...
export const WorksheetView: React.FC<WorksheetViewProps> = ({ worksheet, theme, showKey = false, showDoodles = false, isMathMode = false }) => {
  ...
  const addDoodle = (url: string) => {
    const newDoodle: PlacedDoodle = {
      ...
      url,
      x: 10 + (placedDoodles.length * 5) % 30, // Default positions
  ...
  const titleSizeClass = isPreschool ? 'text-4xl text-left' : isCreative ? 'text-6xl text-center' : (isClassicProfessional ? 'text-base font-bold uppercase tracking-tight' : 'text-3xl font-black');
            ...
            <div className="grid grid-cols-2 gap-8">
              {tracingItems.map((q) => (
                <div key={q.id} className="relative">
                   <DraggableLineRow text={q.correctAnswer} isSmall={true} />
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <>
      ...
      >
        {/* Dynamic Placed Doodles */}
        {placedDoodles.map(doodle => (
          <div 
            key={doodle.id}
            className="absolute group z-50 cursor-move"
            style={{ left: `${doodle.x}%`, top: `${doodle.y}%`, width: '120px' }}
          >
        ...
        <div className={`relative z-10 px-0 ${sectionSpacingClass}`}>
          ...
          {worksheet.diagramImage && (
            <div className="mt-8 mb-12">
               ...
               ) : (isCreative || showDoodles) ? <HandDrawnDivider label="Scientific Observation" /> : <h2 className="text-2xl font-black text-slate-800 border-l-4 border-purple-600 pl-4 py-1 mb-6">Technical Reference</h2>}
